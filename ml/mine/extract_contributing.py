"""
Step 4: Extract file mentions from CONTRIBUTING.md per repo via BigQuery.
...
def main():
    ...
    print(f"Extracting CONTRIBUTING mentions for {len(repo_list)} repos via BigQuery...")
    ...
    for i in tqdm(range(0, len(repo_list), batch_size)):
        ...
        query = f"""
            ...
            WHERE repo_name IN ({repo_filter})
              AND LOWER(path) LIKE '%contributing%'
